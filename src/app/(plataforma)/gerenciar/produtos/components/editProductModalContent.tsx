...
export function EditProductModalContent({
  ...
  useEffect(() => {
    if (product && open) {
      const newSizesArray = product.size ? product.size.split(', ') : [];
      setSizesArray(newSizesArray);
      reset({
        code: product.code,
        name: product.name ?? '',
  ...
  async function handleUpdatedProduct(data: FormSchemaProduct) {
    try {
      await updatedProductFn({
        ...
        size: data.category === 'Roupas' ? sizesArray.join(', ') : '',
    ...
    <DialogContent>
      ...
      {product && (
        <form
          ...
          className="max-h-[50vh] space-y-4 overflow-y-auto overflow-x-hidden px-2"
        >
